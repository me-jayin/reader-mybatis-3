...
public final class MappedStatement {
    ...
    /**
     * 基于传入的入参，生成一个 BoundSql 对象，也就是会根据入参，得到最后的SQL语句以及参数映射集等
     * @param parameterObject
     * @return
     */
    public BoundSql getBoundSql(Object parameterObject) {
        // 通过当前解析后的 SqlSource，来构建一个 BoundSql 对象
        BoundSql boundSql = sqlSource.getBoundSql(parameterObject);
        List<ParameterMapping> parameterMappings = boundSql.getParameterMappings();
        // 由于可能使用了 parameterMapping，并且语句只含有 ? 占位符，因此这里需要判断一下SqlSource构建的 BoundSql 里 parameterMappings 是否为空
        // 如果为空则直接使用 ParameterMap#parameterMappings 来作为最终映射列表
        if (parameterMappings == null || parameterMappings.isEmpty()) {
            boundSql = new BoundSql(configuration, boundSql.getSql(), parameterMap.getParameterMappings(), parameterObject);
        }

        // check for nested result maps in parameter mappings (issue #30)
        // 校验 parameterMapping 中是否使用了 嵌套的resultMap
